const seed = [
  {
    id: 1,
    stock: 0,
    title: "Al final mueren los dos",
    rating: 4.32,
    authors: ["Adam Silvera", "Jeff Smith"],
    price: 3490,
    images: [
      "https://www.tematika.com/media/catalog/Ilhsa/Imagenes/646539.jpg",
    ],
    numberOfOrders: 0,
    genres: ["novella"],
    publishingHouse: "Puck",
    pages: 346,
    weight: 300,
    isbn: "978-987413201-7",
    language: "es",
    description:
      "Imagina un mundo en el que una organización les avisa a las personas la inminencia de su muerte. imagina que empezó la cuenta regresiva. imagina cómo vivirías tu vida¿ en solo un día. ¿estarías listo para decir adiós? al final mueren los dos es un canto a la fuerza arrolladora de la vida, que transformará para siempre tu forma de comprender el tiempo, la amistad y el amor.¿estarías listo para decir adiós? al final mueren los dos es un canto a la fuerza arrolladora de la vida, que transformará para siempre tu forma de comprender el tiempo, la amistad y el amor.¿estarías listo para decir adiós? al final mueren los dos es un canto a la fuerza arrolladora de la vida, que transformará para siempre tu forma de comprender el tiempo, la amistad y el amor.",
    reviews: [
      {
        idUser: 1,
        author: "Matias Dominguez",
        score: 3,
        content:
          "Madeline Miller did what the movie producers of the film Troy (2004) were too cowardly to do; she stayed true to the homosexuality of Homers Iliad rather than writing a censored version of the story which stank of homophobia. Achilles and Patroclus were passionately in love, which resulted in their respective destructions. They were not cousins or man at arms, but soul mates. The watering down of this in the film Troy was an insult to the LGBT community. Nothing more. Nothing less",
        createdAt: "October 26, 2022",
      },
      {
        idUser: 2,
        author: "Matias Dominguez",
        score: 5,
        content:
          "Madeline Miller did what the movie producers of the film Troy (2004) were too cowardly to do; she stayed true to the homosexuality of Homers Iliad rather than writing a censored version of the story which stank of homophobia. Achilles and Patroclus were passionately in love, which resulted in their respective destructions. They were not cousins or man at arms, but soul mates. The watering down of this in the film Troy was an insult to the LGBT community. Nothing more. Nothing less",
        createdAt: "October 26, 2022",
      },
      {
        idUser: 3,
        author: "Matias Dominguez",
        score: 5,
        content:
          "Madeline Miller did what the movie producers of the film Troy (2004) were too cowardly to do; she stayed true to the homosexuality of Homers Iliad rather than writing a censored version of the story which stank of homophobia. Achilles and Patroclus were passionately in love, which resulted in their respective destructions. They were not cousins or man at arms, but soul mates. The watering down of this in the film Troy was an insult to the LGBT community. Nothing more. Nothing less",
        createdAt: "October 26, 2022",
      },
      {
        idUser: 4,
        author: "Matias Dominguez",
        score: 2,
        content:
          "Madeline Miller did what the movie producers of the film Troy (2004) were too cowardly to do; she stayed true to the homosexuality of Homers Iliad rather than writing a censored version of the story which stank of homophobia. Achilles and Patroclus were passionately in love, which resulted in their respective destructions. They were not cousins or man at arms, but soul mates. The watering down of this in the film Troy was an insult to the LGBT community. Nothing more. Nothing less",
        createdAt: "October 26, 2022",
      },
    ],
  },
  {
    id: 2,
    stock: 7,
    title: "Hecha de estrellas",
    authors: ["Ashley Herring Blake"],
    rating: 2.39,
    price: 2899,
    images: ["https://i.postimages.cc/qdbn0hrs/hecha-de-estrellas.jpg"],
    numberOfOrders: 0,
    genres: ["novella"],
    publishingHouse: "vr ya",
    pages: 296,
    weight: 300,
    isbn: "9789877475050",
    language: "es",
    description:
      "Mara y owen son los gemelos más cercanos del mundo. así que cuando él es acusado de violación, ella no sabe qué pensar. ¿puede su hermano ser culpable de algo tan atroz? dividida entre el amor por su familia y su sentido de la justicia, mara deberá hacerle frente a un trauma del pasado que le impide ser libre y ser fuerte para enfrentar la realidad de su presente. con sensibilidad y franqueza, esta novella encara el abuso sexual, la culpa que enfrentan las víctimas y los límites del consentimiento.",
  },
];
    export default seed;
